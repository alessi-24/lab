...
function add_noise(data, noise)
    n = length(data)
    d_spc = Normal(0,noise)
    rng = MersenneTwister(1234)
    data_noise = data + rand(rng, d_spc,n)
    return data_noise
end

function make_noise_data(noise)
    # 読み込み
    read_file_path = "/Users/nishimura/home/lab/3para/data_make/data/spc/spc.h5"
...
function main()
    # ノイズのリスト
    noise_list = [10^(-4.0), 10^(-3.5),10^(-3.0), 10^(-2.5), 10^(-2.0), 10^(-1.5), 10^(-1.0), 10^(-0.5), 10^(0.0), 10^(0.5), 10^(1.0)]
    ...
    for (index, noise) in enumerate(noise_list)
        ...
        # 書き込みpath
        write_file_path = "/Users/nishimura/home/lab/3para/data_make/data/spc/spc_" * string(index) * ".h5"
